"""
Redis Publisher - Used by doc_upload service.
Publishes document IDs to the queue after storing in MongoDB.
"""
import redis
import json
import os
...
class DocumentPublisher:
    ...
    def publish_document(self, document_id: str, filename: str) -> bool:
        ...
        try:
            ...
            # Use LPUSH for queue (FIFO with BRPOP on consumer side)
            self._client.lpush(DOCUMENT_QUEUE, json.dumps(message))
            logger.info(f"Published document {document_id} to queue")
            return True
