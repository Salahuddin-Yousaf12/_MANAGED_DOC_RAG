"""
Redis Subscriber/Worker - Listens for document processing messages.
"""
import redis
import json
import os
...
class DocumentSubscriber:
    ...
    async def start_worker(
        ...
        self._running = True
        logger.info("Document processing worker started - listening for messages")

        while self._running:
            try:
                # BRPOP blocks until message available (5 second timeout)
                ...
                if result:
                    ...
                    logger.info(f"Received message: processing document {document_id} ({filename})")
                    ...
                    # Timeout, no message - yield control
